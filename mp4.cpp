...
#define MP4_GETSTRINGZ( p_str )         \
    ...
    {       \
        ...
        p_str = (char*)malloc( __i_copy__+1 );               \
        if( p_str ) \
        { \
...
{
   int      i_read;
   uint8_t *p_buff = (uint8_t*)malloc(32);
   uint8_t *p_peek = p_buff;
...
static int MP4_ReadBoxSkip( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   {
      uint8_t *p_buff = (uint8_t*)malloc(44);
      uint8_t *p_peek = p_buff;
      int     i_read;
      uint32_t i_fcc;
...
static int MP4_ReadBox_ftyp( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   if( ( p_box->data.p_ftyp->compatible_brands_count = i_read / 4 ) )
   {
      ...
      uint32_t *tab = p_box->data.p_ftyp->compatible_brands =
         (uint32_t*)calloc( p_box->data.p_ftyp->compatible_brands_count, sizeof(uint32_t));
...
static int MP4_ReadBox_mmpu( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   for (i=0;i<(int)p_box->data.p_mmpu->asset_id_length;i++)
   {
...
static int MP4_ReadBox_trun(  stream_t *p_stream, mp4_box_t *p_box )
{
   unsigned int i;
    ...
    p_box->data.p_trun->samples =
      (mp4_descriptor_trun_sample_t*)calloc( p_box->data.p_trun->sample_count, sizeof(mp4_descriptor_trun_sample_t) );
...
static int MP4_ReadBox_stts( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_box->data.p_stts->sample_count =
      (uint32_t*)calloc( p_box->data.p_stts->entry_count, sizeof(uint32_t) );
   p_box->data.p_stts->sample_delta =
      (int32_t*)calloc( p_box->data.p_stts->entry_count, sizeof(uint32_t) );
...
static int MP4_ReadBox_ctts( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_box->data.p_ctts->sample_count =
      (uint32_t*)calloc( p_box->data.p_ctts->entry_count, sizeof(uint32_t) );
   p_box->data.p_ctts->sample_offset =
      (int32_t*)calloc( p_box->data.p_ctts->entry_count, sizeof(uint32_t) );
...
#define es_descriptor p_box->data.p_esds->es_descriptor
    ...
    es_descriptor.decConfigDescr->decoder_specific_info = (uint8_t*)malloc( i_len );
...
static int MP4_ReadBox_avcC( stream_t *p_stream, mp4_box_t *p_box )
{
    ...
    if( p_avcC->avcC > 0 )
    {
        uint8_t * p = p_avcC->p_avcC = (uint8_t*)malloc( p_avcC->avcC );
        if( p )
    ...
    if( p_avcC->sps > 0 )
    {
        p_avcC->sps_length = (uint16_t*)calloc( p_avcC->sps, sizeof( uint16_t ) );
        p_avcC->p_sps = (uint8_t**)calloc( p_avcC->sps, sizeof( uint8_t* ) );
        ...
        for( i = 0; i < p_avcC->sps; i++ )
        {
            ...
            p_avcC->p_sps[i] = (uint8_t*)malloc( p_avcC->sps_length[i] );
            if( p_avcC->p_sps[i] )
    ...
    if( p_avcC->pps > 0 )
    {
        p_avcC->pps_length = (uint16_t*)calloc( p_avcC->pps, sizeof( uint16_t ) );
        p_avcC->p_pps = (uint8_t**)calloc( p_avcC->pps, sizeof( uint8_t* ) );
        ...
        for( i = 0; i < p_avcC->pps; i++ )
        {
            ...
            p_avcC->p_pps[i] = (uint8_t*)malloc( p_avcC->pps_length[i] );
            if( p_avcC->p_pps[i] )
...
static int MP4_ReadBox_stsz( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   if( p_box->data.p_stsz->sample_size == 0 )
   {
      p_box->data.p_stsz->entry_size =
         (uint32_t*)calloc( p_box->data.p_stsz->sample_count, sizeof(uint32_t) );
...
static int MP4_ReadBox_stsc( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_box->data.p_stsc->first_chunk =
      (uint32_t*)calloc( p_box->data.p_stsc->entry_count, sizeof(uint32_t) );
...
static int MP4_ReadBox_stco_co64( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_box->data.p_co64->chunk_offset =
      (uint64_t*)calloc( p_box->data.p_co64->entry_count, sizeof(uint64_t) );
...
static int MP4_ReadBox_stss( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_box->data.p_stss->sample_number =
      (uint32_t*)calloc( p_box->data.p_stss->entry_count, sizeof(uint32_t) );
...
static int MP4_ReadBox_stsh( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_box->data.p_stsh->shadowed_sample_number =
      (uint32_t*)calloc( p_box->data.p_stsh->entry_count, sizeof(uint32_t) );
...
static int MP4_ReadBox_padb( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_box->data.p_padb->reserved1 = (uint16_t*)calloc( count, sizeof(uint16_t) );
   p_box->data.p_padb->pad2 = (uint16_t*)calloc( count, sizeof(uint16_t) );
   p_box->data.p_padb->reserved2 = (uint16_t*)calloc( count, sizeof(uint16_t) );
   p_box->data.p_padb->pad1 = (uint16_t*)calloc( count, sizeof(uint16_t) );
...
static int MP4_ReadBox_elst( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_box->data.p_elst->segment_duration =
      (uint64_t*)calloc( p_box->data.p_elst->entry_count, sizeof(uint64_t) );
   p_box->data.p_elst->media_time =
      (int64_t*)calloc( p_box->data.p_elst->entry_count, sizeof(uint64_t) );
   p_box->data.p_elst->media_rate_integer =
      (uint16_t*)calloc( p_box->data.p_elst->entry_count, sizeof(uint16_t) );
...
static int MP4_ReadBox_cmvd( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   if( !( p_box->data.p_cmvd->data = (uint8_t*)malloc( i_read ) ) )
      MP4_READBOX_EXIT( 0 );
...
static int MP4_ReadBox_chpl( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   for( i = 0; i < p_chpl->chapter; i++ )
   {
      ...
      p_chpl->p_chapter[i].psz_name = (char*)malloc( i_len + 1 );
...
static int MP4_ReadBox_tref_generic( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   if( p_box->data.p_tref_generic->entry_count > 0 )
      p_box->data.p_tref_generic->track_ID = (uint32_t*)calloc( p_box->data.p_tref_generic->entry_count, sizeof(uint32_t) );
...
static int MP4_ReadBox_sdtp( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_sdtp->sample_table = (uint8_t*)calloc( i_sample_count, 1 );
...
static int MP4_ReadBox_tfra( stream_t *p_stream, mp4_box_t *p_box )
{
   ...
   p_tfra->time = (uint32_t*)calloc( i_number_of_entries, size );
   p_tfra->moof_offset = (uint32_t*)calloc( i_number_of_entries, size );

   size = 1 + p_tfra->length_size_of_traf_num; /* size in [|1, 4|] */
   p_tfra->traf_number = (uint8_t*)calloc( i_number_of_entries, size );
   size = 1 + p_tfra->length_size_of_trun_num;
   p_tfra->trun_number = (uint8_t*)calloc( i_number_of_entries, size );
   size = 1 + p_tfra->length_size_of_sample_num;
   p_tfra->sample_number = (uint8_t*)calloc( i_number_of_entries, size );
...
static void get_token( char **ppsz_path, char **ppsz_token, int *pi_number )
{
   ...
   *ppsz_token = (char*)malloc( i_len + 1 );
...
{
   char *psz_dup;
   char *psz_path = (char*)malloc(4096);
   char *psz_token;
